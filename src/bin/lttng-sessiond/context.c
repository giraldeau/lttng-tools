		...
		struct lttng_event_context *ctx, char *channel_name)
{
	...
	case LTTNG_EVENT_CONTEXT_CALLSTACK_KERNEL:
		...
		break;
	case LTTNG_EVENT_CONTEXT_CALLSTACK_USER_FP:
		kctx->ctx.ctx = LTTNG_KERNEL_CONTEXT_CALLSTACK_USER_FP;
		break;
	case LTTNG_EVENT_CONTEXT_CALLSTACK_USER_UNW:
		kctx->ctx.ctx = LTTNG_KERNEL_CONTEXT_CALLSTACK_USER_UNW;
		break;
	default:
